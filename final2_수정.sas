...
/*교육수준 2cat (edu_re)*/
IF year in (1998,2001) & educ in (0:6) THEN do; 
   if educ in (0:4) then edu_re=1;
   else if educ in (5:6) then edu=2;
end;
IF year in (2005:2018) and edu in (1:4) then do;
if edu in (1,2) then edu_re=1;
else if edu in (3,4) then edu_re=2;
end;
...
else if 1980<=birthyear<1990 then birthcohort=4;
...
CLUSTER psu;
WEIGHT wt_pool_2;
DOMAIN birthcohort*SEX*edu_re*AGE;
VAR HE_BMI;
run;
...
else if cohort=4 then bmi_4=bmi;
run;
data B.COHORT_WOMEN;
SET B.COHORT_WOMEN;
if cohort=1 then bmi_5= BMI;
else if cohort=2 then bmi_6= BMI;
else if cohort=3 then bmi_7= BMI;
else if cohort=4 then bmi_8=bmi;
run;
...
proc sgplot data=B.cohort_men;
  ...
  series x=age y=bmi_4/ legendlabel='Birth in 1980-1989' lineattrs=(PATTERN=SOLID COLOR=lightpurple) ;
  keylegend / location=inside position=topright across=1 titleattrs=(family="Times New Roman") valueattrs=(family="Times New Roman");
XAXIS LABEL="Age" values=(17 to 70 by 2) LABELATTRS=(FAMILY="Times New Roman") valueattrs=(FAMILY="Times New Roman") ;
...
proc sgplot data=B.cohort_women;
  series x=AGE y=BMI_5/ legendlabel='Birth in 1950-1959' lineattrs=(PATTERN=SOLID COLOR=lightblue) ;
  series x=AGE y=BMI_6/ legendlabel='Birth in 1960-1969' lineattrs=(PATTERN=SOLID COLOR=lightred) ;
  series x=AGE y=BMI_7/ legendlabel='Birth in 1970-1979' lineattrs=(PATTERN=SOLID COLOR=lightgreen) ;
  series x=age y=bmi_8/ legendlabel='Birth in 1980-1989' lineattrs=(PATTERN=SOLID COLOR=lightpurple) ;
  keylegend / location=inside position=topright across=1 titleattrs=(family="Times New Roman") valueattrs=(family="Times New Roman");
XAXIS LABEL="Age" values=(17 to 70 by 2) LABELATTRS=(FAMILY="Times New Roman") valueattrs=(FAMILY="Times New Roman") ;
yaxis label="Mean BMI (kg/m2)" min=20 max=27 labelpos=top LABELATTRS=(FAMILY="Times New Roman") valueattrs=(FAMILY="Times New Roman");
run;
...
proc sort data=b.graph2_term_men; by term; run;
proc sort data=b.graph2_term_women; by term; run;
