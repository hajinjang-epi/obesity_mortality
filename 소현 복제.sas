libname a "C:\Users\jjoyk\Desktop\Workspace\knhanes_METS\사망연계";
...
libname b "C:\Users\jjoyk\OneDrive\문서\석사\Physical Activity";
run;
...
*if pa_day=0 then BE3_12=0 and BE3_13=0;  /* ~~이러이러하면 어떻게 어떻게 해서 끝내기*/ /* 운동 하루도 안하는 사람들은 결측값이므로 0으로 설정 */
...
pa_day2=BE3_21-1;
*if pa_day2=0 then BE3_22=0 and BE3_23=0;  /* ~~이러이러하면 어떻게 어떻게 해서 끝내기*/ /* 운동 하루도 안하는 사람들은 결측값이므로 0으로 설정 */
/*PA2=중증도 신체활동*/
...
*if walk_day=0 then BE3_32=0 and BE3_33=0;  /* ~~이러이러하면 어떻게 어떻게 해서 끝내기*/ /* 운동 하루도 안하는 사람들은 결측값이므로 0으로 설정 */
...
data b.cohort_v3;
set b.cohort_v2_2;
if modvig_m= . or modvigcat=. or muscle_freq2=. then delete;
run;
/*35723 (97 excluded)*/
...
run;
proc freq data=b.cohort_v3;
tables modvigcat muscle_freq2 modvig_m;
run;
...
run;
/*table 1-age adjusted -macro 이용*/


/*table 2-연령, 성별 보정한 것*/
/*
proc freq data=b.cohort_v2_2;
tables ms_wc*modvig_m ms_TG*modvig_m ms_HDL*modvig_m ms_glu*modvig_m ms_HTN*modvig_m ms_finall*modvig_m;
run;

proc surveymeans data=cohort_v2_2 nomcar;
strata kstrata;
cluster psu;
weight wt_pool_1;
var age;
...
tables modvig_m*ms_all;
run;
proc surveyreg data=cohort_v2_2 nomcar;
strata kstrata;
cluster psu;
weight wt_itvex;
class modvig_m sex;
model ms_final=modvig_m sex age / solution clparm vadjust=none;
run;

proc surveylogistic data=cohort_v2_2 nomcar;
strata kstrata;
cluster psu;
weight wt_itvex;
class modvig_m(ref='0') sex(ref='1')/param=glm;
model ms_final(event='1')= modvig_m sex age/vadjust=none df=infinity;
lsmeans modvig_m / at age=43.738059 om e cl ilink adjust=bon;
 run;*/
proc glm data=b.cohort_final;
class modvig_m sex;
model ms_final=modvig_m age sex; 
lsmeans modvig_m;
run;
...
where sex=2;
class modvig_m;
model ms_glu=modvig_m age; 
lsmeans modvig_m /stderr tdiff pdiff;
estimate 'diff modvig_m0 vs modvig_m1' modvig_m 1 -1 0 0;
estimate 'diff modvig_m0 vs modvig_m2' modvig_m 1 0 -1 0;
estimate 'diff modvig_m0 vs modvig_m3' modvig_m 1 0 0 -1;
run;
quit; 
proc glm data=b.cohort_final;
where sex=2;
class modvig_m;
model ms_final=modvig_m age; 
lsmeans modvig_m df ;
run;
/*table 1에 pa_walk 추가해야 해서, pa_walk 변수 수정*/
data b.cohort_final;
set b.cohort_final;
if pa_walk=0 then  pa_walk_new=2;
if pa_walk=1 then pa_walk_new=1;
run;
...
run;
 ...
 /*table3*/
*5 modvig_m;
*interaction : muscle_freq2*modvigcat;
...
model exit_age1*cancerdeath(0) =  modvig_m  sex educat regioncat occp_new ho_incm marricat_new  pa_walk  muscle_freq2*modvigcat/rl entry=entry_age1;
strata year ;
run;
*model2: lifestyle;
proc phreg data=b.cohort_final;
